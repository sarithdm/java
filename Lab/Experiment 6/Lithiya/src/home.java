...
try {
	...
	rs=stmt.executeQuery("SELECT * FROM bus where source='"+source+"' and destination ='"+destination+"' and month='"+month+"' and date='"+date+"'");
	if(rs.next()){
		seat=rs.getInt("seats");
		if(seat>=Integer.parseInt(count)){
			seat = seat-Integer.parseInt(count);
			PreparedStatement ps=con.prepareStatement("Update bus set seats=? where source='"+source+"' and destination ='"+destination+"' and month='"+month+"' and date='"+date+"'");
			ps.setInt(1, seat);
